	...
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		<div class="flex h-16 items-center justify-between">
			<div class="flex items-center gap-8">
				...
				>
					<div class="relative">
						<div class="absolute -inset-1 rounded-full bg-indigo-500/20 blur opacity-0 group-hover:opacity-100 transition-opacity"></div>
						<img
							src="/logo.png?v=4"
							alt="AKN Logo"
			...
			<div class="flex items-center gap-4">
				...
				{:else if $isAuthenticated}
					<div class="user-menu-container relative">
						<button
							onclick={toggleMenu}
							class="group flex items-center gap-3 rounded-full border border-neutral-800 bg-neutral-900/50 py-1.5 pl-1.5 pr-3 transition-all hover:border-neutral-700 hover:bg-neutral-800/50"
						>
							<div class="h-7 w-7 overflow-hidden rounded-full border border-neutral-700 bg-neutral-800">
								{#if $user?.profilePic}
									<img src={$user.profilePic} alt="Profile" class="h-full w-full object-cover" />
								{:else}
									<div class="flex h-full w-full items-center justify-center text-[10px] font-bold text-neutral-500">
										{($user?.name || '?').charAt(0).toUpperCase()}
									</div>
								{/if}
							</div>
							<span class="max-w-[120px] truncate text-xs font-semibold text-neutral-300 group-hover:text-white">
								{$user?.name || 'My Account'}
							</span>
							<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-neutral-500 transition-transform {isMenuOpen ? 'rotate-180' : ''}"><path d="m6 9 6 6 6-6"/></svg>
						</button>
						...
						{#if isMenuOpen}
							<div
								...
								class="absolute right-0 mt-2 w-64 origin-top-right rounded-2xl border border-neutral-800 bg-neutral-900 p-2 shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none"
							>
								<div class="px-4 py-3 border-b border-neutral-800 mb-2">
									<div class="flex items-center justify-between mb-1">
										<p class="text-xs font-medium text-neutral-500">Logged in as</p>
										<span class="rounded-full px-2 py-0.5 text-[9px] font-bold tracking-tighter uppercase
								...
								<div class="space-y-1">
									...
									{#if $user?.role === 'ADMIN' || $user?.role === 'SUPERADMIN'}
										...
										>
											<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M9 3v18"/><path d="m14 8 2 2-2 2"/></svg>
											Admin Dashboard
										</a>
									{/if}
